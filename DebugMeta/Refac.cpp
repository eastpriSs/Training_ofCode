...
private:
    ...
    std::vector<std::string>
        vecOfManyArguments,// Для количества аргументов    
        LocalVec = {}; // Если имеем дело с List
    
    std::string for_comparison [3];
...
//  ===========================================
        ...
        // -       =========   ==========
            ...
            //===================================================
                ...
                // Если в переменной вызывается функция
                    ...
                    if (varArgumentsCall)
                    {
                        if (symbol_now == ','){ vecOfManyArguments.push_back(argumets); argumets = ""; }
                        ...
                        if (symbol_now == ')')
                        {
                            ...
                            vecOfManyArguments.push_back(argumets);
                            argumets = "";

                            // factorial()
                            if (lastCall == FACTORIAL){
                                int *local = new int; // Возвращение факториала
                                *local = factorial(vecOfManyArguments);
                            ...
                            if (lastCall == LIST_FROM){
                                ...
                                functionFROM(variableName, vecOfManyArguments);
                            }
    ...
    // =====================================================================================================================
                ...
                // Ветвление    
                    if (lastCall == IF_SYN || lastCall == EMPTY){ // ----- last == EMPTY Для облегчения синтаксиса -----  east.
                        // Скармлеваем функции те аргументы, что удалось найти, если вернуло false, то скипаем все, что в условии
                        if(if_ComparisonSupport(vecOfManyArguments) == true){
                            if_ReturnFalse = false;
                        }
                ...
                // Методы с 2мя аргументами
                    ...
                    if (amountOfArgumentsForEachMethod[lastCall] == 2)
                    {
                        ...
                        if (lastCall == SPEED_PRINT){
                            speedPrint(vecOfManyArguments);
                        }
                        // add();
                        if (lastCall == ADD){
                            add(vecOfManyArguments);
                        }
                        // remove();
                        if (lastCall == REMOVE){
                            remove(vecOfManyArguments);
                        }
                    }
                ...
                // For
                    ...
                    if  (lastCall == FOR_SYN || lastCall == EXTRA_SYN_FOR){// ----- last == EXTRA_SYN_FOR Для облегчения синтаксиса -----  east
                        int *l_p = new int; *l_p = -1;
                        // Перекидываем аргументы, в специальный список для for()
                        for (o : vecOfManyArguments){
                            for_comparison[++l_p] = o;
                        }
                        delete l_p;
                        for_loop = true;
                        backIndx = i+1;   
                ...
                if (argumetsAdding){
                    ...
                    if (lastCall == IF_SYN || lastCall == EMPTY || lastCall == FOR_SYN || lastCall == EXTRA_SYN_FOR){ // ----- last == EMPTY Для облегчения синтаксиса -----  east.
                        if (CheckElem(symbol_now, charsForIfComparison) && !stringIsTrue ){
                            vecOfManyArguments.push_back(argumets); // Добавляем num из примера выше
                            argumets = EMPTY;
                            ...
                            if (CheckElem(symbol_next, charsForIfComparison))
                            {
                                ...
                                vecOfManyArguments.push_back(std::to_string(symbol_now + symbol_next));                                                                                    
                                missSymbol = true; // Избаляемся от "==" допустим
                                continue; // Избаляемся от "==" допустим
                            
                            }else{
                                vecOfManyArguments.push_back(std::to_string((int) symbol_now));
                                continue;
                            }
                        }                  
                    }
                    ...
                    if (symbol_now == ')'){
                        ...
                        if(vecOfManyArguments.size() > 0) vecOfManyArguments.push_back(argumets);
...
// Меню перед парсингом файла -> запрос файла
//                               ввод параметор запуска
//                               проверка кфг
